...
static bool streamFile(String path)
{
    ...
    if (LittleFS.exists((char *)(path + ".gz").c_str()) || LittleFS.exists((char *)path.c_str()))
    {
...
void handleNotFound()
{
    ...
    else if (msSystem.msESPServer.uri() != "/upload")
    {
        ...
        {
            message +=
                " " + msSystem.msESPServer.argName(i) + ": " + msSystem.msESPServer.arg(i) + "\n";
        }
    ...
    else
    {
      ...
      <body>\
        <h1>Upload of " + String(msGlobals.ggUploadFileName) +
...
void handleFileUpload()
{
    ...
    if (upload.status == UPLOAD_FILE_START)
    {
        ...
        // *)upload.filename.c_str()); msGlobals.ggUploadFile = SD.open(upload.filename.c_str(),
        // FILE_WRITE); DBG_OUTPUT_PORT.print("Upload: START, filename: ");
