...
public:
  ...
  long deltaMicros = 0; // !J! todo: init? (msGlobals.ggCurrentMicros -
                        // msGlobals.ggLastMicros);

  //  button activity
  bool msBtnActive = false;

  bool powerButtonPressed(void) {
    // had unusual high values on one of the 100x PCBs so this threshold is
    // bigger now
...
#define SYNC_CLICK_TIME_MS 250
  bool menuButtonsSelected() {
    if ((msBtnAHit && (millis() - msBtnBLastHitTime) < SYNC_CLICK_TIME_MS) ||
        ...
        (msBtnALongHit &&
         (millis() - msBtnBLastLongHitTime) < SYNC_CLICK_TIME_MS) ||
        (msBtnBLongHit &&
         (millis() - msBtnALastLongHitTime) < SYNC_CLICK_TIME_MS)) {
      resetButtons();
      return true;
    }
    return false;
