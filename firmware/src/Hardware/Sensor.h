...
#define GSCALE                                                                 \
  8 // Sets full-scale range to +/-2, 4, or 8g. Used to calc real g values.
...
public:
  void step() {
    ...
    for (int i = 0; i < 3; i++) {
      msGlobals.ggAccel[i] =
          (float)msGlobals.ggAccelCount[i] /
          ((1 << 12) /
           (2 * GSCALE)); // get actual g value, this depends on scale being set
    }
#endif
  }
  void initI2C() {
  ...
  byte readRegister(byte addressToRead) {
    ...
    Wire.requestFrom(
        MMA8452_ADDRESS,
        1); // Ask for 1 byte, once done, bus is released by default
  ...
  void MMA8452Standby() {
    ...
    writeRegister(CTRL_REG1,
                  c & ~(0x01)); // Clear the active bit to go into standby
  }
  ...
  void readAccelData(int *destination) {
    ...
    for (int i = 0; i < 3; i++) {
      int gCount =
          (rawData[i * 2] << 8) |
          rawData[(i * 2) +
                  1]; // Combine the two 8 bit registers into one 12-bit number
      gCount >>= 4;   // The registers are left align, here we right align the
                      // 12-bit integer

      // If the number is negative, we have to make it so manually (no 12-bit
      // data type)
      if (rawData[i * 2] > 0x7F) {
        gCount -= 0x1000;
      }
  ...
  // See the many application notes for more info on setting all of these
  // registers:
  ...
  bool setupSensor() {
    ...
    // The default data rate is 800Hz and we don't modify it in this example
    // code

    // more magnetometer ..
    // M_CTRL_REG1: Hybrid mode, OS = 32, Auto Cal when set
    writeRegister(0x5B, (autoCalibrateMagnetometer ? 0x80 : 0x00) | 0x1F);
