...
// PIN_LED_DATA an PIN_LED_CLOCK can not be changed since we use the hardware
// SPI
...
byte lookupBrightness[16] = {B00000000, B00000010, B00000100, B00000111,
                             B00001011, B00010010, B00011110, B00101000,
                             B00110010, B01000001, B01010000, B01100100,
                             B01111101, B10100000, B11001000, B11111111};
...
public:
  ...
  void disableLEDHardware() { pinMode(PIN_LED_ENABLE, INPUT); }
  ...
  void bootSwipe() {
    ...
    for (byte idx = 0; idx < MAX_LEDS; idx++) {
      ...
      setLED(idx, (i2 & 1) ? 255 : 0, (i2 & 2) ? 255 : 0, (i2 & 4) ? 255 : 0,
  ...
  void fastClear() { SPI.writeBytes(clearBuffer, RGB_BUFFER_SIZE + 8); }
  ...
  void setChannel(uint8_t index, uint8_t channelIndex, uint8_t value) {
    if (index >= 0 && index < MAX_LEDS && channelIndex >= 0 &&
        channelIndex < 3) {
#if (LED_TYPE == LED_TYPE_APA102)
      int idx = index << 2;
