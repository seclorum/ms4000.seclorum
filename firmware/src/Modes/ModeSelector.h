...
public:
  void setText(const char *label) {
    ...
    msMagicShakeText.plotTextString((char *)label, msGlobals.ggtBitmap6x8,
                                    tPos);
  ...
  // stop the mode
  virtual void stop(void) { lPOVMode.setImage(NULL); }
  ...
  int select() {
    ...
    if (msSystem.msButtons.msBtnPwrHit) {
      ...
      for (byte i = 0; i < 4; i++) {
        msSystem.msLEDs.setLED(_posIdx, 255, 255, 255, msGlobals.ggBrightness);
    ...
    if (lPOVMode.step()) {
      ...
      for (byte j = 0; j < 16; j++) {
        if (j == _posIdx) {
          msSystem.msLEDs.setLED(j, r, g, b, msGlobals.ggBrightness);
        } else {
