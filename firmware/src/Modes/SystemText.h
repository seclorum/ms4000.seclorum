...
public:
  SystemTextMode() { modeName = "Values"; }

  void setText(char *label, char *value, MSColor color, char *value2,
               MSColor color2) {
  ...
  // stop the mode
  void stop(void) { lPOVMode.setImage(NULL); }
  ...
  bool step() {
    ...
    if (msSystem.msButtons.msBtnPwrHit) {
      ...
      if (new_mode == MS4_App_System_Mode_POWER_HI) {
        ...
        msGlobals.ui.timeoutHighPower =
            predefinedPowerModes[ptHiMode].powertime;
        msSystem.Settings.setUIConfig(&msGlobals.ui);
      }
    ...
    if (needsTextUpdate) {
      ...
      if (new_mode == MS4_App_System_Mode_VERSION) {
        setText((char *)String("VERSION").c_str(), (char *)MS3KOS_VERSION,
                aWHITE);
      }
      if (new_mode == MS4_App_System_Mode_IP) {
        ...
        setText((char *)String("VOLT").c_str(),
                (char *)String(msSystem.batteryVoltage).c_str(),
                isBatteryLow ? aRED : aGREEN);
      } else if (new_mode == MS4_App_System_Mode_CALIBRATION) {
        setText((char *)String("CALIB").c_str(),
                (char *)String(msGlobals.batVoltCalibration).c_str(),
                isBatteryLow ? aRED : aGREEN);
        ...
        if (!isConnectedToAP) {
          ...
          msMagicShakeText.plotTextString((char *)"N", msGlobals.ggtBitmap4x5,
                                          tPos);
        } else {
          msGlobals.ggtBitmap4x5.color = aGREEN;
          msMagicShakeText.plotTextString((char *)"Y", msGlobals.ggtBitmap4x5,
                                          tPos);
        }
        lPOVMode.setImage(&msMagicShakeText);
      }
