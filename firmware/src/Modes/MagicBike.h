 ...
 *
 *		sub-mode: rear
                                if (wifi connected) to front shifter, blink when
turning
 *
**/
#include <MQTT.h>
...
private:
  ...
  char packetBuffer[UDP_TX_PACKET_MAX_SIZE +
                    1]; // buffer to hold incoming packet,
...
public:
  ...
  virtual bool step(void) {
    ...
    if (packetSize) {
      msSystem.slogln("UDP rec'd: " + String(packetSize) +
                      " from:" + bikeUDP.remoteIP().toString() +
                      " port: " + String(bikeUDP.remotePort()));
      msSystem.slogln("  [..] to:" + bikeUDP.destinationIP().toString() +
                      " port: " + String(bikeUDP.localPort()));
