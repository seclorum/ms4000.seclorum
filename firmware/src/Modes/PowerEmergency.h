...
// 				   b) Check for any button press: if true,
// poweroff
//				   c) Blink a single LED on and off at 125
// intervals
//                 d) if the cable is plugged in, abort the 60-second countdown
//                 and
...
public:
  PowerEmergencyMode() { modeName = "pwr!"; };
  ...
  virtual bool step() {
    ...
    // power down if we have been up for too long (a), or if the user presses a
    // button (b) ..
    if (msGlobals.ggCurrentMillis >= (modeStartTime + POWERDOWN_LIVE_TIME) ||
        (msSystem.msButtons.msBtnAHit) || (msSystem.msButtons.msBtnBHit) ||
        (msSystem.msButtons.msBtnPwrHit)) {
      msSystem.powerDown();
    }

    // blink the LED (c)
    if (msGlobals.ggCurrentMillis < (blinkedAt + BLINK_PERIOD)) {
      msSystem.msLEDs.setLED(LEDidx, (LEDidx & 1) ? 255 : 0, 0, 0,
      ...
      msSystem.slog(String(modeStartTime));
      msSystem.slog(":");
      msSystem.slogln(String(blinkedAt));
    }
