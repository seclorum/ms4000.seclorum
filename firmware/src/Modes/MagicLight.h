...
private:
  ...
  int d = 10;
  uint8_t lookup[6][3] = {{0, 1, 2}, {1, 0, 2}, {2, 0, 1},
                          {0, 2, 1}, {1, 2, 0}, {2, 1, 0}};
...
public:
  ...
  bool step() {
    ...
    if (_light.mode == MS4_App_Light_Mode_RAINBOW) {
      if (frame % pDelay == 0) {
        msSystem.msLEDs.fillLEDs(0, 0, 0, msGlobals.ggBrightness);
        msSystem.msLEDs.setLED((xx + 0 * 3) & MAX_LEDS, 255, 0, 0,
                               msGlobals.ggBrightness);

        msSystem.msLEDs.setLED((xx + 1 * 3) & MAX_LEDS, 255, 255, 0,
                               msGlobals.ggBrightness);
        msSystem.msLEDs.setLED((xx + 2 * 3) & MAX_LEDS, 0, 255, 0,
                               msGlobals.ggBrightness);

        msSystem.msLEDs.setLED((xx + 3 * 3) & MAX_LEDS, 0, 255, 255,
                               msGlobals.ggBrightness);
        msSystem.msLEDs.setLED((xx + 4 * 3) & MAX_LEDS, 0, 0, 255,
                               msGlobals.ggBrightness);
  ...
  void startToEndChannel(uint8_t start, uint8_t end, int d, int channel,
                         int color) {
    int i;
  ...
  void startToEndZigZag(uint8_t start, uint8_t end, int d, uint8_t r, uint8_t g,
                        uint8_t b) {
    int i;
    uint8_t lastEnd = end;
